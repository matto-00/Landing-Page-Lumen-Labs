# GLASS KIT — Referencia de Diseño
## Propósito
Este archivo contiene un kit profesional de componentes glassmorphism.
Claude Code debe usarlo como REFERENCIA VISUAL Y DE CÓDIGO para las cards
de la sección singularidad. NO importar ni ejecutar nada — solo inspiración
y tokens de diseño.

## Reglas de uso
- SÍ usar: variables CSS, clases .glass, backdrop-filter, box-shadow multicapa,
  badges con dot pulsante, animaciones cascade-in y float, sistema de sombras
  con reflection-top, tokens de spacing y radius
- NO usar: las fuentes del kit (ya tenemos Inter, JetBrains Mono, Instrument Serif)
- NO importar: el script liquidGlassKit.js ni ningún JS del kit
- NO copiar: tabs, accordion, dropdowns, data tables — no los necesitamos

## Lo que buscamos aplicar en las cards JUKUS
Cards con backdrop-filter blur real que muestre el fondo del BH detrás.
Badges de estado con dot pulsante (igual que .glass-badge--aqua del kit).
Animación de entrada cascade-in al hacer scroll hacia la singularidad.
Sombras multicapa con tinte de color (no flat shadows).
Sistema de capas: base → elevated → floating, no todo al mismo z-plane.

## A continuación: los 3 bloques del kit (HTML, CSS, JS)
